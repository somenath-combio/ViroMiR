...
# ── Check RNAhybrid availability ──────────────────────────────────
import shutil
RNAHYBRID = shutil.which("RNAhybrid") or shutil.which("rnahybrid")
print(f"RNAhybrid : {RNAHYBRID or 'NOT FOUND'}")
print(f"miRanda   : {MIRANDA}")

# ── Build cross-virus negative set ───────────────────────────────
print("\n── Building cross-virus test set ───────────────────────────")

vb_raw = pd.read_csv(VIRBASE)
vm_raw = pd.read_csv(VIRMIRNA)

# Get positives with full sequences
...
print(f"  Positives with sequences : {len(pos_all)}")

# Group sequences by virus
virus_seqs = {}
for _, r in pos_all.iterrows():
    v = str(r['virus'])
    if v not in virus_seqs:
        virus_seqs[v] = []
...
# Build pairs: positive = real, negative = same miRNA + different virus mRNA
pairs = []
for _, r in pos_all.iterrows():
    ...
    # Cross-virus negative: same miRNA, random mRNA from DIFFERENT virus
...
print(f"\n  Sampled for benchmark    : {len(test_pairs)} pairs")
...
for i, pair in enumerate(test_pairs):
    ...
    sc_intarna.append(-dg)   # more negative = stronger = higher score
    ...
    if rh is not None:
        sc_rnahybrid.append(-rh)  # more negative = stronger
    else:
...
sc_viromir = []
for i, pair in enumerate(test_pairs):
    dg = -sc_intarna[i]  # recover original dg
